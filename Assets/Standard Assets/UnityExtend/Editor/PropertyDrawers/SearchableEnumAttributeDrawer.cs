...
public class SearchablePopup : PopupWindowContent
{
	...
	/// </param>
	public static void Show(Rect activatorRect, IList<string> options, int current, Action<int> onSelectionMade)
	{
		// copy string to clipboard on right click
		if (Event.current.type == EventType.ContextClick && activatorRect.Contains(Event.current.mousePosition))
		{
			var menu = new GenericMenu();
			menu.AddItem(new GUIContent("Copy to Clipboard"), false, () => EditorGUIUtility.systemCopyBuffer = options[current]);
			menu.ShowAsContext();
		}
	...
	private SearchablePopup(IList<string> names, int currentIndex, Action<int> onSelectionMade)
	{
	...
	{
		return new Vector2(base.GetWindowSize().x + 300,
